...
float sprite(vec2 spr, vec2 size, vec2 uv)
{
    ...
    float pixels = texture2D(u_FontTexture, texImageSegUV).r;
...
void main()
{
    vec4 srce = texture2D(u_SourceTexture, v_texCoord);
    ...
    vec2 textUv = vec2(1.0 - v_texCoord.y, v_texCoord.x) * scale;
